...
export function analyzeUPITransactions(transactions) {
  ...
  // If no valid transactions, return null
  ...
  for (let txn of validTransactions) {
  ...
  const contactCounts = {};
  for (let txn of validTransactions) {
    if (!contactCounts[txn.to]) {
      contactCounts[txn.to] = 0;
    }
    contactCounts[txn.to]++;
  }
